...
export function deriveDispersion(history) {
  // REFINEMENT 1 (review P2): a settled ladder's transition row carries a degenerate point-mass
  // IQR — left in, the settlement step reads as a spurious 'converging' narrative on a resolved
  // market. The band NARRATION must come from traded days only.
  const rows = stripResolutionRows(ordered(history));
  const kind = rows.length ? rows[rows.length - 1].kind : null;
...
export function deriveDeltas(history, thresholds) {
  // REFINEMENT 1 (review P2): the transition row's settled 0/1 rungs would otherwise flow into the
  // Δ columns (LadderThresholdTable) as a huge "1-day move" that was settlement, not trading.
  const rows = stripResolutionRows(ordered(history));
 ...
 *  'no_history' is DELIBERATELY excluded from retry — it means every leg answered and none has
 *  history (never-traded tokens, or a board past CLOB's retention horizon), which is a terminal
 *  outcome, not a failure; retrying it would loop forever for no gain. Only null|'failed' retry —
 *  this is by construction, not an oversight. */
 ...
 *  DEPRECATED (fix/cron-batch-ledger, INC 7): writes the row only at the end of a run, which loses
 *  it entirely on a hard kill. Kept for back-compat (no other caller today) — the live snapshot
 *  path now uses insertCronRunStart + updateCronRunFinal via lib/cron-batch.mjs.
...
/** Insert the START row of a cron_runs ledger entry (migration 0016, INC 7 soft-budget ledger:
 *  fix/cron-batch-ledger). Landed BEFORE any market is served — a Vercel hard kill mid-batch (no
 *  graceful-shutdown hook, doc-confirmed) still leaves a queryable trace instead of nothing.
 *  Totals are zeroed here; the real counts land via updateCronRunFinal. status='started' — a row
 *  that never advances past this is the signature of a hard kill (see updateCronRunFinal).
 *  @param {{started_at:string, run_date:string, snapshot_hour:number}} row
 *  @returns {Promise<number>} the inserted row's id, to pass to updateCronRunFinal */
export async function insertCronRunStart({ started_at, run_date, snapshot_hour }) {
  const { data, error } = await db().from('cron_runs').insert({
    started_at, run_date, snapshot_hour, status: 'started',
    total: 0, skipped_already: 0, skipped_resolved: 0, success: 0, failed: 0,
    failures: [], backfill_retried: [],
  }).select('id').single();
  if (error) throw new Error(`market-history cron_runs start: ${error.message}`);
  return data.id;
}

/** Patch a cron_runs ledger row (migration 0016) with its final counts, status, and completed_at.
 *  Caller (lib/cron-batch.mjs runSnapshotBatch) treats a failure here as NON-FATAL but LOUD: the
 *  run's actual work already happened, so this function throwing must never lose the summary —
 *  the caller wraps this in try/catch and logs loud on failure rather than swallowing it quietly.
 *  @param {number} ledgerId
 *  @param {{total:number, skipped_already:number, skipped_resolved:number, success:number,
 *    failed:number, failures:Array, backfill_retried:Array, status:'completed'|'truncated',
 *    completed_at:string}} patch */
export async function updateCronRunFinal(ledgerId, patch) {
  const { error } = await db().from('cron_runs').update(patch).eq('id', ledgerId);
  if (error) throw new Error(`market-history cron_runs final update: ${error.message}`);
}
...
export function leanHistoryRow(raw) {
  const { markets, iqr, outcomes, high_series, low_series, exclusivity, ...cols } = raw ?? {};
  ...
  // exclusivity (5.6 guard) is stored ONLY on guarded rows — absent means exclusive, the same
  // convention the full read carries; exclusivityMode's `?? 'exclusive'` default is CORRECT here,
  // not a mask. Must be destructured above: left in `raw` it would leak into `...cols` as a stray
  // top-level field where exclusivityMode (which reads record.snapshot.derived.exclusivity) can't see it.
  if (exclusivity != null) derived.exclusivity = exclusivity;
...
const LEAN_SHAPE_PATHS = {
  ...
  // per-outcome chart lines + exclusivity (5.6 guard: {verdict, basis} whole block — anti-recurrence,
  // and the monitor's S3 wants basis.filtered_sum later) so latestModeSegment can trim on this path too
  categorical: 'outcomes:record->snapshot->derived->outcomes, exclusivity:record->snapshot->derived->exclusivity',

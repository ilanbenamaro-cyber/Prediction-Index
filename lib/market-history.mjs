...
export function detectJumps(history, { threshold = JUMP_THRESHOLD, recentDays = JUMP_RECENT_DAYS } = {}) {
  const rows = latestModeSegment(ordered(history)); // 5.6: a guard transition is not a jump
...
export function deriveVelocity(history) {
  const rows = latestModeSegment(ordered(history)); // 5.6: never derive across a guard transition
  const kind = rows.length ? rows[rows.length - 1].kind : null;
...
const LEAN_BASE_COLS = 'snapshot_date, snapshot_hour, kind, implied_median, implied_mean, confidence_tier, confidence_score, reliability_tier, reliability_score, liquidity_tier, liquidity_score, probability, touch_range_lo, touch_range_hi, dominant_outcome, dominant_prob, raw_sha256';

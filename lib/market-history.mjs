...
/** PURE: collapse history rows to ONE row per UTC date (the nearest-US-peak capture wins — so a
 *  cron capture at hour 15/19 beats an hour-0 backfill reconstruction of the same day), ascending
 *  by date. Since migration 0009 the unique key is (market_id, snapshot_date, snapshot_hour), so a
 *  date can legitimately carry a backfill row PLUS cron rows; every consumer wants one point per
 *  day. Exported for the dedup tests and reused by ordered(). */
...
/** PURE: reshape a lean projection row (markets/iqr pulled out of the record JSONB server-side)
 *  back into the readHistory row shape, so every derive fn (ladderProbAt/iqrWidth/deriveChartSeries)
 *  consumes it unchanged. Exported for the full≡lean equivalence test. */
export function leanHistoryRow(raw) {
  const { markets, iqr, ...cols } = raw ?? {};
  return { ...cols, record: { snapshot: { derived: { markets: markets ?? [], iqr: iqr ?? null } } } };
}

/**
 * Read the last `days` of history for ONE market as a LEAN projection: the promoted scalar
 * columns plus ONLY the two record sub-paths the display derivations consume —
 * derived.markets (per-threshold Δ / movers / chart lines) and derived.iqr (dispersion).
 ...
 */
export async function readHistoryLean(marketId, days = 90) {
  const cutoff = new Date(Date.now() - days * DAY_MS).toISOString().slice(0, 10);
  const { data, error } = await db().from('market_history')
    .select('snapshot_date, snapshot_hour, kind, implied_median, implied_mean, confidence_tier, confidence_score, reliability_tier, reliability_score, liquidity_tier, liquidity_score, probability, touch_range_lo, touch_range_hi, dominant_outcome, dominant_prob, raw_sha256, markets:record->snapshot->derived->markets, iqr:record->snapshot->derived->iqr')

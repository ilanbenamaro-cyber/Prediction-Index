 ...
 */
export async function writeRecord(marketId, record, lifecycle, config, { replace = false } = {}) {
  ...
  // `replace: true` (seed/re-seed path only) updates the existing row instead of ignoring the
  // conflict — a frozen record's fetched_at never changes, so without it a re-seed after a
  // schema reshape is a silent no-op and the stored row keeps the stale shape forever.

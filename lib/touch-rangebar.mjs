...
export function rangeBarLayout(low, high, levels, geom = {}) {
  ...
  const { x0 = 0, W = RANGEBAR_W, yAbove = Y_ABOVE, yBelow = Y_BELOW, narrowFrac = NARROW_FRAC, domain = null } = geom;
  // `domain` (optional [min,max]) scopes the axis explicitly — e.g. to the ACTIVE touch levels with
  // padding, so dead deep strikes don't stretch the scale. Defaults to the levels' own min/max.
  const min = domain ? domain[0] : Math.min(...levels);
  const max = domain ? domain[1] : Math.max(...levels);
 ...
 */
/**
 * Resolve a touch range bound to a band edge, DIRECTION-AWARE (a null bound is NOT always
 * "extend to the plot edge"). The direction depends on where the 50% crossover fell relative to
 * the quoted strike ladder, which is read from the series' probabilities:
 *   • finite value                          → { value, extend:false, unresolved:false }
 *   • LOW null, series all ≥ 0.5 ("< $min")  → floor is BELOW the bottom strike
 *                                            → { value:null, extend:true }  (band extends left)
 *   • LOW null, series all < 0.5  ("> $max") → floor is ABOVE the top low strike (Anthropic)
 *                                            → { value:max(levels), unresolved:true } (anchor, no extend)
 *   • HIGH null, series all ≥ 0.5 ("> $max") → cap is ABOVE the top strike
 *                                            → { value:null, extend:true }  (band extends right)
 *   • HIGH null, series all < 0.5 ("< $min") → cap is BELOW the bottom high strike
 *                                            → { value:min(levels), unresolved:true } (anchor, no extend)
 * The label direction (`boundLabel` in core/touch-record.js) is derived from the SAME `allAbove`
 * test, so the band edge and its "> $X"/"< $X" label always agree. `unresolved` edges get a dashed
 * marker + an explicit caption (the floor/cap is beyond the quoted strikes, not a precise value).
 * @param {number|null} value @param {{level:number,prob:number}[]} series @param {'low'|'high'} kind
 */
export function resolveBound(value, series, kind) {
  if (value != null) return { value, extend: false, unresolved: false };
  if (!Array.isArray(series) || series.length === 0) return { value: null, extend: true, unresolved: false };
  const levels = series.map((s) => s.level);
  const allAbove = series.every((s) => s.prob >= 0.5);
  if (kind === 'low') {
    return allAbove
      ? { value: null, extend: true, unresolved: false }              // "< $min": floor below → extend left
      : { value: Math.max(...levels), extend: false, unresolved: true }; // "> $max": floor above → anchor at ladder top
  }
  return allAbove
    ? { value: null, extend: true, unresolved: false }               // "> $max": cap above → extend right
    : { value: Math.min(...levels), extend: false, unresolved: true }; // "< $min": cap below → anchor at ladder bottom
}

/**
 * Uniform samples across the scoped axis domain [dMin,dMax] for the crosshair (Item-4 alignment
 * fix): N anchors evenly spaced in BOTH pixel-x and price, so the crosshair reads the price
 * LINEARLY off the axis — exactly matching the tick scale — instead of off sparse per-level anchors
 * (which left the axis margins, e.g. the pixels between the plot edge and the lowest quoted level,
 * reporting the clamped end level). Returns { xs, prices } (parallel arrays for bracket()+lerpAt()).
 */
export function buildAxisSamples(dMin, dMax, plotLeft, plotWidth, n = 96) {
  const N = Math.max(2, n | 0);
  const xs = [], prices = [];
  for (let k = 0; k < N; k++) {
    const f = k / (N - 1);
    xs.push(plotLeft + f * plotWidth);
    prices.push(dMin + f * (dMax - dMin));
  }
  return { xs, prices };
}

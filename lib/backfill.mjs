 ...
 *   deps.setStatus(slug, status, through)  (status: pending|done|failed|no_history; through = earliest date)
...
export async function backfillMarket({ slug, deps, log = console }) {
  ...
  try {
    ...
    // Three-way status decision on assembled.length, then (for the non-empty case) the
    // pre-existing write-outcome rule:
    //   assembled=0 + legErrors>0  → OUTAGE: every leg that threw degraded to history:[], so an
    //     empty assembly here is indistinguishable from "no market ever existed" UNLESS we track
    //     the error signal separately — 'failed' (retryable; the cron's needsBackfill self-heals it).
    //   assembled=0 + legErrors=0  → GENUINE-EMPTY: every leg answered 200 with no history (a
    //     never-traded token, or a board past CLOB's ~Nov-2023 retention horizon) — 'no_history'.
    //     TERMINAL by design (excluded from needsBackfill's retry set) — marking it 'failed' would
    //     retry forever, and it must never masquerade as 'done' ("no history exists" is not the
    //     same claim as "successfully backfilled").
    //   assembled>0                → byte-identical to the pre-existing rule: wrote nothing AND
    //     something failed → failed; otherwise done (zero-write with no failures is a legitimate
    //     "already fully backfilled" outcome — e.g. every row was a cron-precedence duplicate).
    const status = assembled.length === 0
      ? (legErrors > 0 ? 'failed' : 'no_history')
      : (written === 0 && failed > 0 ? 'failed' : 'done');

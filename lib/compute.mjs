...
import { parseGammaArr, num, settledYesStr, settledYesNum } from '../core/settlement.js';
 ...
 * Returns { record, lifecycle, config, writeReplace }. `writeReplace` is true ONLY when the record
 * keeps the prior's fetched_at (a degraded resolved-transition freeze, or a CLOSED_PENDING freeze
 * with a prior) — those writes MUST declare replace:true or lib/cache.writeRecord's upsert silently
 * no-ops on the (market_id, fetched_at) key. Throws { code } on a 404/409-class issue. Never returns
 * an unvalidated record (validateRecord runs before return).
 */
export async function computeMarketRecord({ id, prior = null }) {
  ...
  // Non-OPEN: there are no live midpoints to pull.
  if (lifecycle.state !== LIFECYCLE.OPEN) {
    if (lifecycle.state === LIFECYCLE.RESOLVED) {
      // RESOLVED (with OR without a prior): settled truth, not a frozen live estimate — see
      // resolveTransition. A prior degrades to a freeze (writeReplace:true) only when no rung has
      // a parseable settled price; prior-less still 409s on that same failure.
      const legs = await fetchMarketMeta(survivalBootstrap(id));
      return resolveTransition('survival', legs, id, lifecycle, prior,
        () => ({ record: freezePriorRecord(prior, lifecycle), lifecycle, config: prior.snapshot?.lifecycleConfig ?? null }));
    }
    // CLOSED_PENDING: no settled truth exists yet — freeze semantics.
    if (!prior) {
      const e = new Error(`"${id}" is ${lifecycle.state} with no prior record to freeze`);
      e.code = 409;
      throw e;
    }
    // Same no-op class the resolved freeze guards against: fetched_at is unchanged from the
    // prior, so the write MUST declare replace:true or the lifecycle stamp never persists.
    return { record: freezePriorRecord(prior, lifecycle), lifecycle, config: prior.snapshot?.lifecycleConfig ?? null, writeReplace: true };
  }
  ...
  return { record, lifecycle, config, writeReplace: false };
}
...
// parseGammaArr / num / settledYesStr / settledYesNum now live in core/settlement.js —
// hoisted (INC settled-leg-pricing) so the live pricing chain (core/fetch.js priceLeg)
// and these resolved-no-prior builders read settled truth through ONE definition.
...
/**
 * The RESOLVED-lifecycle transition, shared by all 5 shapes (fix/resolved-transition-settled-truth).
 *
 * Before this fix, a market resolving out from under a cached prior called freezePriorRecord, which
 * keeps the prior's `fetched_at` — and writeRecord upserts on (market_id, fetched_at) with
 * ignoreDuplicates, so the RESOLVED row silently never landed: the market stayed OPEN in the cache
 * forever and every serve repeated probe→freeze→no-op (live victims: world-cup-winner,
 * which-continent-will-win-the-world-cup). The fix: a RESOLVED transition ALWAYS tries to build
 * SETTLED TRUTH via buildMinimalResolvedRecord (the same builder the no-prior "browse a resolved
 * market for the first time" path already uses) — this naturally gets a NEW fetched_at (`now`), so
 * the write is a genuine insert, not a same-key upsert. Only when NO leg has a parseable settled
 * price (buildMinimalResolvedRecord's typed 409) do we DEGRADE to the old freeze behavior — logged
 * LOUD (never a silent fallback) — and that freeze must declare writeReplace:true, because it keeps
 * the prior's fetched_at and would otherwise silently no-op for the exact reason this fix exists.
 * A prior-less RESOLVED (first-ever browse) still 409s on unparseable settlement — there is nothing
 * to degrade TO.
 *
 * @param {'binary'|'survival'|'bucket_pmf'|'directional_touch'|'categorical'} shape
 * @param {object} meta shape-specific gamma meta (same as buildMinimalResolvedRecord)
 * @param {string} id the event slug
 * @param {object} lifecycle the RESOLVED lifecycle
 * @param {object|null} prior the last cached record, or null
 * @param {() => {record:object, lifecycle:object, config:object}} degradeFn builds the per-shape freeze
 * @returns {{record:object, lifecycle:object, config:object, writeReplace:boolean}}
 */
function resolveTransition(shape, meta, id, lifecycle, prior, degradeFn) {
  try {
    return { ...buildMinimalResolvedRecord(shape, meta, id, lifecycle), writeReplace: false };
  } catch (err) {
    if (!prior || err.code !== 409) throw err; // no prior to degrade to, or an unexpected failure
    console.warn('[resolved-transition] DEGRADED to freeze', JSON.stringify({ id, error: err.message }));
    return { ...degradeFn(), writeReplace: true };
  }
}
...
async function computeBinaryRecord({ id, prior = null }) {
  ...
  if (lifecycle.state !== LIFECYCLE.OPEN) {
    if (lifecycle.state === LIFECYCLE.RESOLVED) {
      // RESOLVED (with OR without a prior): settled truth from gamma's outcomePrices — a resolved
      // binary market ALWAYS caches its final settlement, never a stale frozen live estimate (see
      // resolveTransition). A prior degrades to a freeze only when settled prices are unparseable.
      const meta = await fetchBinaryMeta({ event_slug: id });
      return resolveTransition('binary', meta, id, lifecycle, prior, () => freezeBinaryPriorRecord(prior, lifecycle, id));
    }
    // CLOSED_PENDING (trading ended, UMA unconfirmed → no settled outcome and no live prices)
    // genuinely can't be built → still 409 with no prior.
    if (!prior) {
      const e = new Error(`"${id}" is ${lifecycle.state} with no prior record to freeze`);
      e.code = 409;
      throw e;
    }
    return { ...freezeBinaryPriorRecord(prior, lifecycle, id), writeReplace: true };
  }
...
async function computeBucketPmfRecord({ id, prior = null }) {
  ...
  if (lifecycle.state !== LIFECYCLE.OPEN) {
    if (lifecycle.state === LIFECYCLE.RESOLVED) {
      // RESOLVED (with OR without a prior): settled truth from the per-bucket outcomePrices — see
      // resolveTransition. A prior degrades to a freeze only when no bucket has a parseable
      // settled price.
      const meta = await fetchBucketMeta({ event_slug: id });
      return resolveTransition('bucket_pmf', meta, id, lifecycle, prior,
        () => ({ record: freezePriorRecord(prior, lifecycle), lifecycle, config: prior.snapshot?.lifecycleConfig ?? null }));
    }
    // CLOSED_PENDING (no settled outcome yet) still 409s with no prior.
    if (!prior) {
      const e = new Error(`"${id}" is ${lifecycle.state} with no prior record to freeze`);
      e.code = 409;
      throw e;
...
async function computeTouchRecord({ id, prior = null }) {
  ...
  if (lifecycle.state !== LIFECYCLE.OPEN) {
    if (lifecycle.state === LIFECYCLE.RESOLVED) {
      // RESOLVED (with OR without a prior): settled truth from the HIGH/LOW touch outcomes — see
      // resolveTransition. A prior degrades to a freeze only when no leg has a parseable
      // settled price.
      const meta = await fetchTouchMeta({ event_slug: id });
      return resolveTransition('directional_touch', meta, id, lifecycle, prior, () => freezeTouchPriorRecord(prior, lifecycle, id));
    }
    // CLOSED_PENDING still 409s with no prior.
    if (!prior) {
      const e = new Error(`"${id}" is ${lifecycle.state} with no prior record to freeze`);
      e.code = 409;
      throw e;
    }
    return { ...freezeTouchPriorRecord(prior, lifecycle, id), writeReplace: true };
  }
...
async function computeCategoricalRecord({ id, prior = null }) {
  ...
  if (lifecycle.state !== LIFECYCLE.OPEN) {
    if (lifecycle.state === LIFECYCLE.RESOLVED) {
      // RESOLVED (with OR without a prior): settled truth from the per-outcome prices — see
      // resolveTransition. A prior degrades to a freeze only when no outcome has a parseable
      // settled price.
      const meta = await fetchCategoricalMeta({ event_slug: id });
      return resolveTransition('categorical', meta, id, lifecycle, prior, () => freezeCategoricalPriorRecord(prior, lifecycle, id));
    }
    // CLOSED_PENDING still 409s with no prior.
    if (!prior) {
      const e = new Error(`"${id}" is ${lifecycle.state} with no prior record to freeze`);
    ...
    return { ...freezeCategoricalPriorRecord(prior, lifecycle, id), writeReplace: true };
  }

...
function touchSettlementLabel(high, low, unit = '') {
  const hit = [
    ...high.filter((p) => p.prob >= 0.5).map((p) => `HIGH $${p.level}${unit}`),
    ...low.filter((p) => p.prob >= 0.5).map((p) => `LOW $${p.level}${unit}`),
  ];
...
function buildMinimalBucketPmf(bucketMeta, id, lifecycle) {
  ...
  const config = defaultConfigForLadder(markets.map((m) => m.threshold), {
    id, event_slug: id, name: bucketMeta.title || id, platform: 'polymarket',
